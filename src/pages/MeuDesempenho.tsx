...
const MeuDesempenho: React.FC = () => {
  ...
  const loadMeuDesempenho = async () => {
    if (!user?.id) return

    try {
      setLoading(true)

      // Carregar dados do aluno autenticado usando o ID do usuário
      const alunoData = await alunosApi.buscarPorIdUsuario(user.id)
      setAluno({
        id: alunoData.id,
        nome: alunoData.nome,
      ...
      const inscricoesData = await inscricoesApi.buscarPorAluno(alunoData.id, 0, 1000)
      const inscricoesList = inscricoesData.content || inscricoesData || []
      setInscricoes(inscricoesList)

      // Carregar presenças do aluno
      const presencasData = await presencasApi.listarPorAluno(alunoData.id, 0, 1000)
      ...
      <Layout userName={user?.name} userRole={user?.role} onLogout={logout}>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-[var(--fh-muted)]">Carregando seu desempenho...</p>
          </div>
        </div>
      </Layout>
    )
      ...
      <Layout userName={user?.name} userRole={user?.role} onLogout={logout}>
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <p className="text-[var(--fh-muted)]">Dados do aluno não encontrados</p>
          </div>
        </div>
      </Layout>
    )
        ...
        <div className="bg-[var(--fh-card)] rounded-xl shadow-sm border border-[var(--fh-border)] p-6 space-y-6">
          ...
          {inscricoes.length > 0 ? (
            ...
            <div className="text-center py-12 text-[var(--fh-muted)]">
              ...
              <p>Você não possui inscrições ativas</p>
            </div>
          )}
        </div>
